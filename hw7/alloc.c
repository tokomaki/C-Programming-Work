 ...
 * 05/09/2017
 * Pseudocode for freef:
 * if p is less than allocbuf or greater than allocbuf + ALLOCSIZE
 *   print error message
 *   return
 * Assign pointer
 * Initialize pointer to struct blockl
 * Make pointer to right block
 * if block is not being used
 *   print error message
 *   return
 * Update left and right block tags
 * Update sizes
 * Point rightb to block of block before it
 * Initialize temporary left block pointer
 * Point to left block
 * Call enchain if there is no free left block
...
void freef(char *p)
{
   /* you write the  code for the freef function here */
    ...
    p = p - TAGSIZE;
    struct blockl *leftb = (struct blockl *)p;
    struct blockr *rightb = (struct blockr *)(p + leftb->size - TAGSIZE);
    if (leftb->tag != USEDTAG || rightb->tag != USEDTAG) {
        printf("freef: block being freed is not in use, %p\n", (void *) p);
        return;
    }

    leftb->tag = (rightb->tag = FREETAG);
    rightb->size = leftb->size;

    rightb = (struct blockr*)(p - TAGSIZE);
    struct blockl *temp;
    if ((char *) rightb >= allocbuf && rightb->tag == FREETAG) {
        temp = (struct blockl*)(p - rightb->size);
        p -= rightb->size;
        coalesce(temp, leftb);
        leftb = temp;
    }
    ...
    temp = (struct blockl *)(p + leftb->size);
